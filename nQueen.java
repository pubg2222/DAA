...
public class nQueen {
    ...
    // to save each arrangement present in board of characters, we first convert it
    // as board of string and then save the list in all boards (which is list of
    // lists of string)
    public static void saveBoard(char[][] board, List<List<String>> allBoards) {
        String row = "";
        // this will contain the arrangement of board as string
        List<String> newBoard = new ArrayList<>();
        // traversing through the board of characters and saving each row as string in
        // newBoard
        for (int i = 0; i < board.length; i++) {
            row = "";
    ...
    public static void solve(char[][] board, List<List<String>> allBoards, int col) {
        // if column length becomes equal to board.length, that implies all the columns
        // have been explored and function solve is ready with its solution.
        if (col == board.length) {

        // when we have one of the ways to put queens, we save it as a list called
        // "board" in list of lists "allboards" that contains all the possible
        // solutions.
        ...
        // if the queen is safe to be put in the cell, we put 'Q' in that cell to know
        // that queen has been placed
            if (isSafe(row, col, board)) {
                board[row][col] = 'Q';

        // we again call the solve function by incrementing the column by 1.
                solve(board, allBoards, col + 1);
        // while backtracking, we put '.' in each cell showing that no queen has been
        // placed there as in to again, to explore other solution or moves we need a
        // clear board.
                board[row][col] = '.';
            }
        }
    ...
    public static void solution(int n) {
        ...
        // each board would be of character so that we can put 'Q' to refer to queen
        // placement and '.' to refer that no queen is placed there.
